...
if(isset($_POST['page'])){
  ...
  //get number of rows
  $queryNum = $DB_con->query("SELECT COUNT(*) as postNum FROM `faculty_stats` JOIN `faculties` ON `faculties`.`facultyNo`=`faculty_stats`.`facultyNo` JOIN `faculty_den` ON `faculties`.`FacultyID`=`faculty_den`.`FacultyID` JOIN `department` ON `faculties`.`dept`=`department`.`dept_id`");
  $resultNum = $queryNum->fetch_assoc();
  $rowCount = $resultNum['postNum'];
  ...
  //get rows
  $query = $DB_con->query("SELECT * FROM `faculty_stats` JOIN `faculties` ON `faculties`.`facultyNo`=`faculty_stats`.`facultyNo` JOIN `faculty_den` ON `faculties`.`FacultyID`=`faculty_den`.`FacultyID` JOIN `department` ON `faculties`.`dept`=`department`.`dept_id` ORDER BY date_checked DESC LIMIT $limit");
