...
if (isset($_GET['DID']) && is_numeric($_GET['DID']) && $_GET['DID'] > 0) {

$DID = $_GET['DID'];
$res = "SELECT * FROM `students_den` JOIN `students` ON `students`.`studentNo`=`students_den`.`studentNo` JOIN `program` ON `students`.`program`=`program`.`program_id` WHERE DID=".$_GET['DID'];
...
$DID = $_GET['DID'];
$den_res = mysqli_query($DB_con,"SELECT * FROM `students_den` WHERE DID = '$DID' AND `date_checked`");
