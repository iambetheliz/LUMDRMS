    ...
    <ul id="country-list">
        ...
        if($result->num_rows > 0) {
            foreach($result as $row) { ?>
                <li onClick="selectCountry('<?php echo $row["first_name"]." ".$row['middle_name']." ".$row['last_name']; ?>');"><a href="/LUMDRMS/students/medical_form.php?StudentID=<?php echo $row['StudentID'];?>"><?php echo $row["first_name"]." ".$row['middle_name']." ".$row['last_name']; ?></a>
                </li>
                <?php 
            }
        } else {
